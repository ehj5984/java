	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 99999; i++) {
			...
			if (userInputString.equals("1")) {
				...
				System.out.println(" 1. 전체 유권자 수 : " + s1.size() + " 명 ");
				...
				System.out.println(" 2. 유권자 평균 나이 : " + avgAge + " 세 ");
				...
				for (int j = 0; j < party.size(); j++) {
					System.out.println("이름 :" + party.get(j).name + " (주소:" + party.get(j).address + ") 의 선호 정당 -->> "
							+ party.get(j).party);

				}

			} else if (userInputString.equals("6")) {
				System.out.println("<< 유권자 선호 정당 통계 >>");

				double favParty = (double) party.size() / s1.size();

				System.out.println("1. 선호 정당 입력 비율 : " + favParty);
